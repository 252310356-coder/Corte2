...
*más simple.
 * Además, asegura que el denominador sea positivo, ajustando el signo del numerador si es
**necesario.
 */
 ...
 */
 ...
 $nuevoNumerador = ($this->numerador * ($mcm / $this->denominador)) + ($otraFraccion->numerador * ($mcm / $otraFraccion->denominador));
 ...
 */
 ...
 // Método para elevar la fracción a una potencia dada
 public function potencia(int $exponente): Fraccion {
 if ($exponente <= 0) {
    $nuevoNumerador = (int)pow($this->denominador, abs($exponente));
    $nuevoDenominador = (int)pow($this->numerador, abs($exponente));
 } else {
 ...
 public function fraccion_porcentaje (): string {
